...
static void fix_stat(const char *path, struct stat *s)
{
    ...
    if (canned_config) {
        ...
        fs_config(path, is_dir, target_out_path, &s->st_uid, &s->st_gid, &s->st_mode, &capabilities);
    }
}
...
//    fprintf(stderr, "_eject %s: mode=0%o\n", out, s->st_mode);
    fprintf(fp_out, "%06x%08x%08x%08x%08x%08x%08x"
...
static void _archive_dir(char *in, char *out, int ilen, int olen)
{
    ...
    if (names == NULL) {
      die("failed to allocate dir names array (size %d)\n", size);
