...
#ifndef MYTEMP_FAILURE_RETRY
/* Used to retry syscalls that can return EINTR. */
#define MYTEMP_FAILURE_RETRY(exp) ({         \
    int _rc;                      \
    do {                                   \
        _rc = (exp);                       \
...
static int fs_config_open(int dir, const char *target_out_path)
{
    ...
    if (target_out_path && *target_out_path) {
        ...
        if (asprintf(&name, "%s%s", target_out_path, (dir ? conf_dir : conf_file) + skip_len) != -1) {
            fd = MYTEMP_FAILURE_RETRY(open(name, O_RDONLY | O_BINARY));
            free(name);
        }
    }
    if (fd < 0) {
        fd = MYTEMP_FAILURE_RETRY(open(dir ? conf_dir : conf_file, O_RDONLY | O_BINARY));
    }
    return fd;
}
               ...
               unsigned *uid, unsigned *gid, unsigned *mode, uint64_t *capabilities)
{
    ...
    if (fd >= 0) {
        ...
        while (MYTEMP_FAILURE_RETRY(read(fd, &header, sizeof(header))) == sizeof(header)) {
            char *prefix;
            ...
            if (MYTEMP_FAILURE_RETRY(read(fd, prefix, remainder)) != remainder) {
                free(prefix);
