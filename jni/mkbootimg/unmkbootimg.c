...
int unpack_main(boot_img_hdr *hdr, int isMTKSpecialMode)
{
    void *file_data = 0;
    ...
    memcpy(hdr, file_data, sizeof(struct boot_img_hdr));
    ...
    if(hdr->ramdisk_size != 0) {
        ...
        if (isMTKModel) {
            // unsigned int ramdisk_size = *((unsigned int*)file_data[offset + 4]);
            unsigned int ramdisk_size = *((unsigned int *) &file_data[offset+4]);
            offset += 512;
